  ...
  const requestPermissionsAndStart = async () => {
    ...
    try {
      ...
      if (backgroundStatus !== 'granted') {
        console.log('[LocationGate] ⚠️ Permiso background no concedido en el diálogo del sistema');
        
        Alert.alert(
          '⚠️ ACCIÓN REQUERIDA: Activar ubicación SIEMPRE',
          Platform.OS === 'android' 
            ? 'IMPORTANTE:\n\n1. El sistema solo mostró "Permitir cuando esté en uso"\n2. DEBES ir a Ajustes manualmente\n3. Seleccionar esta app\n4. Tocar "Permisos" → "Ubicación"\n5. Cambiar a "PERMITIR TODO EL TIEMPO"\n\nSin esto, Gerencia NO podrá ver tu ruta cuando cierres la app.'
            : 'Para que la app funcione correctamente, debes seleccionar "Siempre" en los ajustes de ubicación de esta app.',
          [
            {
              text: 'Ir a Ajustes AHORA',
              onPress: async () => {
                if (Platform.OS === 'ios') {
                  await Linking.openURL('app-settings:');
                } else if (Platform.OS === 'android') {
                ...
                setErrorMessage('RECUERDA: Debes cambiar a "Permitir todo el tiempo" en Ajustes.');
              },
            },
          ]
        );
        setRequestingPermissions(false);
        setErrorMessage('⚠️ Ubicación configurada solo "cuando esté en uso". Debes cambiarla a "Permitir todo el tiempo" desde Ajustes.');
        return;
      }
      ...
      <View style={styles.content}>
        ...
        <View style={styles.instructionsContainer}>
          <View style={styles.instructionRow}>
            ...
            <Text style={styles.instructionText}>Activa los servicios de ubicación (GPS)</Text>
          </View>
          <View style={styles.instructionRow}>
            <MapPin size={20} color="#6B7280" />
            <Text style={styles.instructionText}>Acepta el permiso cuando aparezca el diálogo</Text>
          </View>
          <View style={styles.instructionRow}>
            <MapPin size={20} color="#DC2626" />
            <Text style={[styles.instructionText, { color: '#DC2626', fontWeight: '700' as const }]}>LUEGO deberás ir a Ajustes y cambiar a &quot;Permitir todo el tiempo&quot;</Text>
          </View>
        </View>
