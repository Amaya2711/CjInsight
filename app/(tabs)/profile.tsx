...
export default function ProfileScreen() {
  ...
  const updateLocation = async () => {
    ...
    try {
      ...
      console.log("[Profile] 🖥️  Platform.OS:", Platform.OS);
      ...
      if (Platform.OS === 'android' || Platform.OS === 'web') {
        console.log("[Profile] 🛣️  Registrando punto en CUADRILLA_RUTA (Android/Web)...");
        const { insertCuadrillaRuta } = await import('@/services/cuadrillaRuta');
        const { getPeruTimestamp } = await import('@/utils/dateUtils');
        
        const peruTime = getPeruTimestamp();
        console.log("[Profile] 🕐 Hora de Perú:", peruTime.fecha, peruTime.hora);
        
        const rutaResult = await insertCuadrillaRuta({
          cuadrilla_id: user.id,
          latitud: latitude,
          longitud: longitude,
          timestamp: peruTime.timestamp,
            ...
            {isTracking && currentLocation && (
              <View style={styles.locationCard}>
                ...
                <View style={styles.platformRow}>
                  <Text style={styles.platformLabel}>Platform.OS:</Text>
                ...
                {(Platform.OS === 'android' || Platform.OS === 'web') && (
                  <Text style={styles.rutaNote}>
                    ✅ Grabando en CUADRILLA_RUTA cada 5s
                  </Text>
                )}
              </View>
            )}
